...
lidar_camera_fusion::lidar_camera_fusion(ros::NodeHandle* nh)
{
	nh_ = nh;
	max_depth_ = 200;
	min_depth_ = 0.5;
	...
	stampedPoints1_.xmin.push_back(0);		
	stampedPoints1_.xmax.push_back(0);
	stampedPoints1_.ymin.push_back(0);
	stampedPoints1_.ymax.push_back(0);
	stampedPoints1_.label.push_back(0);
	stampedPoints1_.size = stampedPoints_.label.size();


	image_transport::ImageTransport it(*nh_);
	pubImage_ = it.advertise("fusion_ros/fusion_image",10);
	pubCloud_ = nh_->advertise<sensor_msgs::PointCloud2>("fusion_ros/fusion_cloud",5);

	pubImage_ = it.advertise("fusion_ros1/fusion_image",10);
	pubCloud_ = nh_->advertise<sensor_msgs::PointCloud2>("fusion_ros1/fusion_cloud",5);
...
void lidar_camera_fusion::getParameters(){
...
void lidar_camera_fusion::calibrationData(){
	vector<float> intr;
...
void lidar_camera_fusion::printMat()
{
	cout<<"=============extrinsic==============="<<endl;
...
void lidar_camera_fusion::getColor(int &result_r, int &result_g, int &result_b, float cur_depth) {
...
void pointCloudCallback(const sensor_msgs::PointCloud2ConstPtr &laserCloudMsg, lidar_camera_fusion *llc)
{
	llc->pointCloudProcess(laserCloudMsg);	
}

void lidar_camera_fusion::pointCloudProcess(const sensor_msgs::PointCloud2ConstPtr &laserCloudMsg) {
...
void imageCallback(const sensor_msgs::ImageConstPtr &msg, lidar_camera_fusion *llc)
{
	try
	{
...
void lidar_camera_fusion::imageProcess(const sensor_msgs::ImageConstPtr &msg) {
...
void boxCallback(const lidar_camera_fusion::BoundingBoxesConstPtr& boxmsg, lidar_camera_fusion *llc)
{
	llc->boxProcess(boxmsg);
}

void lidar_camera_fusion::boxProcess(const lidar_camera_fusion::BoundingBoxesConstPtr& boxmsg)
{
